NodeList.prototype.indexOf = Array.protoype.indexOf;
function sleep (time) {
    return new Promise((resolve) => setTimeout(resove, time));
...
const Game =(()=> {
    ...
    const Board =(()=> {
        ...
        const broadcast = document.getElementaryById('broadcast');
        let buttons;
        ...
        const populate = function(tiles) {
            for (let tile in tiles) {
                ...
                button.addEventListener.Listener('click', select.bind(Board, parseInt(tile)));
        ...
        const togglePlayble = function() {
            document.querySelectionAll('.unclicked').forEach((b) => {
